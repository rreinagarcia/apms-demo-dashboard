...
// Small class sensitivity so horizon ranking can shift (demo realism)
...
// Horizon-specific PoF (0–100 index) - intentionally changes weights by horizon
export function pofIndex(asset: AssetLike): Record<Horizon, number> {
  const deg = 100 - asset.health; // degradation proxy
...
export function expectedExposureRMm(asset: AssetLike, horizon: Horizon) {
  const pof = pofIndex(asset)[horizon]; // 0–100
  const cost = failureCostRMm(asset.assetClass); // RM m
...
export type InterventionOption = {
  ...
  costRMm: number;
  pofMultiplier: number; // apply to exposure: exposureAfter = exposureBefore * multiplier
...
// Generates CAPEX + OPEX options from predictive signals (demo but believable)
export function interventionOptions(asset: AssetLike): InterventionOption[] {
  // baseline exposure at 12m is what annual planning typically uses (can change in UI)
  const baseExposure12 = expectedExposureRMm(asset, "12m");

  // keep options reasonable: low-risk assets might have only monitoring (but we’ll still allow)
  // costs scale with class and a bit with exposure so larger risks feel more expensive
  const classCost = failureCostRMm(asset.assetClass);
  const c = confidence(asset.assetClass);

  // OPEX: faster, moderate improvement
  const opexCost = Number(Math.max(0.3, Math.min(6, 0.15 * classCost + 0.05 * baseExposure12)).toFixed(1));
  const opexMultiplier =
    asset.risk >= 70 ? 0.65 : asset.risk >= 45 ? 0.72 : 0.80; // reduce exposure 20–35%

  // CAPEX: slower, bigger improvement
  const capexCost = Number(Math.max(2, Math.min(45, 0.65 * classCost + 0.25 * baseExposure12)).toFixed(1));
  const capexMultiplier =
    asset.risk >= 70 ? 0.25 : asset.risk >= 45 ? 0.35 : 0.45; // reduce exposure 55–75%
